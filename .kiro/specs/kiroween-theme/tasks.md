...
- [x] 6. Boot Sequence Animation
  ...
  - Create BootSequence JavaScript class in glitch-effects.js
...
- [ ]* 6.1 Write unit tests for boot sequence
...
- [ ] 7. Spooky Translation System
  - Create `app/static/i18n/ja_spooky.json` with horror/cyberpunk-themed translations
  - Translate common terms (save → "魂を固定", loading → "霊を召喚中...", etc.)
  - Translate login page terms (title → "NECRO-TERMINAL ACCESS", etc.)
  - Translate animal status terms (protected → "保護プロトコル", etc.)
  - Update i18n loading logic in `app/static/js/i18n.js` to select ja_spooky.json when Kiroween Mode is enabled
...
- [ ]* 7.3 Write unit tests for translation system
  - Test ja_spooky.json loads in Kiroween Mode
  - Test standard locale files load in standard mode
  - Test specific translation keys return spooky values
...
### Optional Tasks
Tasks marked with `*` are optional testing tasks. They provide comprehensive test coverage but are not required for core functionality. The implementation can proceed without these tests, though they are recommended for production quality.
