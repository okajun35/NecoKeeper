"""
generate_qr_card MCP Tool

This module provides the generate_qr_card tool for the MCP server, allowing
Claude to generate single QR card PDFs for registered cats.
...
def register_tool(mcp: FastMCP, api_client: NecoKeeperAPIClient) -> None:
    """
    Register the generate_qr_card tool with the MCP server.
    ...
    async def generate_qr_card(
        ...
        指定された猫のQRカード付きPDF（A6サイズ）を生成し、ローカルファイルシステムに保存します。
        PDFは tmp/qr/qr_card_{animal_id}.pdf に保存されます。
        ...
        Example:
            >>> result = await generate_qr_card(animal_id=42)
            >>> print(result["pdf_path"])
            tmp/qr/qr_card_42.pdf
        """
        try:
            ...
            logger.info(f"Generating single QR card PDF for animal_id={animal_id}")

            # Call NecoKeeper API to generate PDF
            # Requirements 2.2: Call /api/automation/pdf/qr-card endpoint
            pdf_content = await api_client.generate_qr_card_pdf(animal_id)
            ...
            # Save PDF to file
            # Requirements 2.3: Save to tmp/qr/qr_card_{animal_id}.pdf
            pdf_path = output_dir / f"qr_card_{animal_id}.pdf"
            pdf_path.write_bytes(pdf_content)

            logger.info(f"Successfully saved QR card PDF to {pdf_path}")
            ...
            raise handle_api_error(e, "generating single QR card PDF") from e
